...
//------------------------------------------------------------------------------
volatile unsigned int txData;                        // UART internal variable for TX
...
void main(void)
{
  int i;
    ...
    for (;;)
    {
      ...
      __bis_SR_register(LPM0_bits); // sets LPM0_bits into the status register.
